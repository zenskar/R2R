...
from shared.abstractions.graph import GraphCreationSettings, GraphEnrichmentSettings, GraphEntityDeduplicationSettings, GraphEntityDeduplicationType
...
# TODO - Fix naming convention to read `GraphService` instead of `GraphService`
...
class GraphService(Service):
    ...
    @telemetry_event("graph_relationships_extraction")
    async def graph_relationships_extraction(
        self,
        document_id: UUID,
        generation_config: GenerationConfig,
        chunks_per_extraction: int,
        ...
        **kwargs,
        ...
        try:
            ...
            logger.info(
                f"GraphService: Processing document {document_id} for KG extraction"
            )
            ...
            relationships = await self.pipes.graph_relationships_extraction_pipe.run(
                input=self.pipes.graph_relationships_extraction_pipe.Input(
                    message={
                        "document_id": document_id,
                        "generation_config": generation_config,
                        "chunks_per_extraction": chunks_per_extraction,
            ...
            logger.info(
                f"GraphService: Finished processing document {document_id} for KG extraction"
            )
            ...
            logger.error(f"GraphService: Error in kg_extraction: {e}")
            await self.providers.database.set_workflow_status(
                id=document_id,
                status_type="extraction_status",
    ...
    async def kg_entity_description(
        ...
        **kwargs,
        ...
        logger.info(
            f"GraphService: Running kg_entity_description for document {document_id}"
        )
        ...
        logger.info(
            f"GraphService: Found {entity_count} entities in document {document_id}"
        )
        ...
        for i in range(num_batches):
            logger.info(
                f"GraphService: Running kg_entity_description for batch {i+1}/{num_batches} for document {document_id}"
            )
            ...
            logger.info(
                f"GraphService: Completed kg_entity_description for batch {i+1}/{num_batches} for document {document_id}"
            )
        ...
        logger.info(
            f"GraphService: Completed kg_entity_description for document {document_id} in {time.time() - start_time:.2f} seconds",
        )
    ...
    async def kg_entity_deduplication(
        ...
        collection_id: UUID,
        graph_id: UUID,
        graph_entity_deduplication_type: GraphEntityDeduplicationType,
    ...
    async def kg_extraction(  # type: ignore
        ...
        **kwargs: Any,
        ...
        grouped_chunks = [
            chunks[i : i + chunks_per_extraction]
            for i in range(0, len(chunks), chunks_per_extraction)
        ]
    ...
    async def _extract_kg(
        ...
        messages = await self.providers.database.prompt_handler.get_message_payload(
            task_prompt=self.providers.database.config.graph_creation_settings.graphrag_relationships_extraction_few_shot,
            task_inputs={
